...
export default function NoteApp({onHome}) {
  ...
  const sendFeedback=()=>{if(!feedbackText.trim())return;const info=`\n\n---\nCategory: ${feedbackType}\nBrowser: ${navigator.userAgent}\nPage: ${pages[currentPage]?.name||'Unknown'}`;const subject=`[Ink Notes Feedback] ${feedbackType.charAt(0).toUpperCase()+feedbackType.slice(1)}`;const body=feedbackText+info;window.open(`mailto:nilay.service@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`,'_self');setShowFeedback(false);setFeedbackText("");};
