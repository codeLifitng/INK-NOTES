...
export default function NoteApp({onHome}) {
  ...
  const handleRemoteData=useCallback((msg)=>{
    ...
    else if(msg.t==='shape'){ctx.save();const ss={x:msg.x1,y:msg.y1},pos={x:msg.x2,y:msg.y2};if(msg.tool===T.LINE){ctx.strokeStyle=msg.c;ctx.lineWidth=msg.s;ctx.lineCap="round";ctx.beginPath();ctx.moveTo(ss.x,ss.y);ctx.lineTo(pos.x,pos.y);ctx.stroke();}else if(msg.tool===T.ARROW)drawArrow(ctx,ss.x,ss.y,pos.x,pos.y,msg.c,msg.s);else if(msg.tool===T.RECT){ctx.strokeStyle=msg.c;ctx.lineWidth=msg.s;ctx.lineCap="round";ctx.strokeRect(ss.x,ss.y,pos.x-ss.x,pos.y-ss.y);}else if(msg.tool===T.DIAMOND)drawDiamond(ctx,ss.x,ss.y,pos.x-ss.x,pos.y-ss.y,msg.c,msg.s);else if(msg.tool===T.CIRCLE){const rx=Math.abs(pos.x-ss.x)/2,ry=Math.abs(pos.y-ss.y)/2;ctx.strokeStyle=msg.c;ctx.lineWidth=msg.s;ctx.lineCap="round";ctx.beginPath();ctx.ellipse(ss.x+(pos.x-ss.x)/2,ss.y+(pos.y-ss.y)/2,rx,ry,0,0,Math.PI*2);ctx.stroke();}ctx.restore();}
  ...
  const createRoom=()=>{const code=Math.random().toString(36).substring(2,8).toUpperCase();setRoomCode(code);setPeerStatus('hosting');const p=new Peer('inknotes-'+code);peerRef.current=p;p.on('connection',(conn)=>{connRef.current=conn;conn.on('open',()=>{setPeerStatus('connected');const cv=canvasRef.current;if(cv)conn.send({t:'sync',image:cv.toDataURL(),texts:tiRef.current});});conn.on('data',handleRemoteData);conn.on('close',()=>{connRef.current=null;setPeerStatus('hosting');});});p.on('error',(e)=>console.error('Peer:',e));};
  const joinRoom=()=>{if(!joinCode.trim())return;setPeerStatus('joining');const p=new Peer();peerRef.current=p;p.on('open',()=>{const conn=p.connect('inknotes-'+joinCode.trim().toUpperCase());connRef.current=conn;conn.on('open',()=>setPeerStatus('connected'));conn.on('data',handleRemoteData);conn.on('close',()=>{connRef.current=null;setPeerStatus('idle');});});p.on('error',(e)=>{console.error('Peer:',e);setPeerStatus('idle');});};
