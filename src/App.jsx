...
export default function NoteApp({onHome}) {
  ...
  const [calendarOpen,setCalendarOpen]=useState(false),[now,setNow]=useState(new Date()),[calMonth,setCalMonth]=useState(new Date().getMonth()),[calYear,setCalYear]=useState(new Date().getFullYear());
  const [dragOverFolder,setDragOverFolder]=useState(null);
  const [showFeedback,setShowFeedback]=useState(false),[feedbackText,setFeedbackText]=useState(""),[feedbackType,setFeedbackType]=useState("suggestion");
  ...
  const handleDown=(e)=>{e.preventDefault();if(pinchRef.current)return;
    ...
    if(tool===T.STAMP){const loc=navigator.language||'en-US';const stampText=new Date().toLocaleString(loc,{weekday:'short',year:'numeric',month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});const nid=Date.now();const nobj={id:nid,x:pos.x,y:pos.y,text:stampText,color,fontSize};setTextInputs(p=>[...p,nobj]);broadcast({t:'txt_add',obj:nobj});setTool(T.SELECT);setSelectedText(nid);return;}
  ...
  const toolDock=[{id:T.HAND,icon:I.hand,tip:"Hand (H)"},{id:T.SELECT,icon:I.select,tip:"Lasso (V)"},"sep",{id:T.PEN,icon:I.pen,tip:"Pen (P)"},{id:T.HIGHLIGHTER,icon:I.highlighter,tip:"Highlight"},{id:T.ERASER,icon:I.eraser,tip:"Eraser (E)"},{id:T.TEXT,icon:I.text,tip:"Text (T)"},{id:T.STAMP,icon:I.calendar,tip:"Stamp (I)"},"sep",{id:T.LINE,icon:I.line,tip:"Line (L)"},{id:T.ARROW,icon:I.arrow,tip:"Arrow (A)"},{id:T.RECT,icon:I.rect,tip:"Rect (R)"},{id:T.DIAMOND,icon:I.diamond,tip:"Diamond (D)"},{id:T.CIRCLE,icon:I.circle,tip:"Ellipse (O)"}];
  ...
  return (
    <div style={{width:"100vw",height:"100vh",display:"flex",flexDirection:"column",background:th.bg,fontFamily:'"DM Sans",system-ui,sans-serif',overflow:"hidden",userSelect:"none",WebkitUserSelect:"none",WebkitTouchCallout:"none",WebkitTextSizeAdjust:"100%"}}>
      ...
      {/* Feedback Modal */}
      {showFeedback&&<div style={{position:"fixed",inset:0,background:dark?"rgba(0,0,0,.6)":"rgba(44,36,24,.3)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"none"}} onClick={()=>setShowFeedback(false)}><div onClick={e=>e.stopPropagation()} style={{background:th.surface,borderRadius:"24px",padding:"36px",maxWidth:"420px",width:"90%",boxShadow:th.shadowLg,border:`1px solid ${th.border}`}}>
        <div style={{display:"flex",alignItems:"center",gap:"14px",marginBottom:"20px"}}><div style={{width:"44px",height:"44px",borderRadius:"14px",background:th.accentGrad,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",flexShrink:0}}>{I.feedback}</div><div><h2 style={{margin:0,fontSize:"17px",color:th.text,fontWeight:700,fontFamily:'"Literata",Georgia,serif'}}>Send Feedback</h2><p style={{margin:"2px 0 0",fontSize:"11px",color:th.textMuted}}>Opens a GitHub issue with your feedback</p></div></div>
        <label style={{display:"block",fontSize:"10px",fontWeight:700,color:th.textMuted,letterSpacing:".8px",marginBottom:"8px",textTransform:"uppercase"}}>Category</label>
        <div style={{display:"flex",gap:"6px",marginBottom:"16px",flexWrap:"wrap"}}>{["bug","suggestion","feature","other"].map(t=>(<button key={t} onClick={()=>setFeedbackType(t)} style={{padding:"6px 14px",borderRadius:"20px",border:feedbackType===t?`2px solid ${th.accent}`:`1.5px solid ${th.border}`,background:feedbackType===t?th.accentSoft:"transparent",color:feedbackType===t?th.accent:th.textSecondary,fontSize:"11px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",textTransform:"capitalize"}}>{t==="feature"?"Feature Request":t}</button>))}</div>
        <label style={{display:"block",fontSize:"10px",fontWeight:700,color:th.textMuted,letterSpacing:".8px",marginBottom:"6px",textTransform:"uppercase"}}>Message</label>
        <textarea autoFocus value={feedbackText} onChange={e=>setFeedbackText(e.target.value)} placeholder="Tell us what you think..." onKeyDown={e=>{if(e.key==="Escape")setShowFeedback(false);}} style={{width:"100%",padding:"12px 14px",borderRadius:"12px",border:`1.5px solid ${th.border}`,background:th.surfaceHover,color:th.text,fontSize:"13px",outline:"none",boxSizing:"border-box",fontFamily:"inherit",minHeight:"120px",resize:"vertical",lineHeight:"1.5"}} onFocus={e=>e.target.style.borderColor=th.accent} onBlur={e=>e.target.style.borderColor=th.border}/>
        <div style={{display:"flex",gap:"10px",marginTop:"16px"}}><button onClick={()=>setShowFeedback(false)} style={{flex:1,padding:"12px",borderRadius:"12px",border:`1px solid ${th.border}`,background:"transparent",cursor:"pointer",fontSize:"13px",fontWeight:600,color:th.textSecondary,fontFamily:"inherit"}}>Cancel</button><button onClick={sendFeedback} disabled={!feedbackText.trim()} style={{flex:2,padding:"12px",borderRadius:"12px",border:"none",cursor:feedbackText.trim()?"pointer":"not-allowed",background:feedbackText.trim()?th.accentGrad:th.surfaceActive,color:feedbackText.trim()?"#fff":th.textMuted,fontSize:"13px",fontWeight:700,fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}>{I.send} Open GitHub Issue</button></div>
      </div></div>}
      ...
      {/* Calendar Dropdown */}
      {calendarOpen&&<div onPointerDown={e=>e.stopPropagation()} style={{position:"fixed",top:"56px",left:"50%",transform:"translateX(-50%)",zIndex:9998,background:th.surface,borderRadius:"20px",padding:"20px",boxShadow:th.shadowLg,border:`1px solid ${th.border}`,width:"300px"}}>
        {(()=>{const today=new Date();const isToday=(d)=>d===today.getDate()&&calMonth===today.getMonth()&&calYear===today.getFullYear();
          const firstDay=new Date(calYear,calMonth,1).getDay();const daysInMonth=new Date(calYear,calMonth+1,0).getDate();const daysInPrev=new Date(calYear,calMonth,0).getDate();
          const monthName=new Date(calYear,calMonth).toLocaleDateString(locale,{month:'long',year:'numeric'});
          const weekDays=Array.from({length:7},(_,i)=>new Date(2024,0,i).toLocaleDateString(locale,{weekday:'narrow'}));
          const cells=[];for(let i=0;i<firstDay;i++)cells.push({d:daysInPrev-firstDay+i+1,cur:false});for(let i=1;i<=daysInMonth;i++)cells.push({d:i,cur:true});const rem=7-cells.length%7;if(rem<7)for(let i=1;i<=rem;i++)cells.push({d:i,cur:false});
          const prevMonth=()=>{if(calMonth===0){setCalMonth(11);setCalYear(y=>y-1);}else setCalMonth(m=>m-1);};
          const nextMonth=()=>{if(calMonth===11){setCalMonth(0);setCalYear(y=>y+1);}else setCalMonth(m=>m+1);};
          const goToday=()=>{setCalMonth(today.getMonth());setCalYear(today.getFullYear());};
          const tzOff=now.getTimezoneOffset();const tzH=Math.floor(Math.abs(tzOff)/60);const tzM=Math.abs(tzOff)%60;const tzStr=`UTC${tzOff<=0?'+':'-'}${tzH}${tzM?':'+String(tzM).padStart(2,'0'):''}`;
          return<>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"14px"}}>
              <button onClick={prevMonth} style={{background:"none",border:"none",cursor:"pointer",color:th.textSecondary,fontSize:"18px",padding:"4px 8px",borderRadius:"8px"}}>‹</button>
              <button onClick={goToday} style={{background:"none",border:"none",cursor:"pointer",fontSize:"14px",fontWeight:700,color:th.text,fontFamily:'"Literata",Georgia,serif'}}>{monthName}</button>
              <button onClick={nextMonth} style={{background:"none",border:"none",cursor:"pointer",color:th.textSecondary,fontSize:"18px",padding:"4px 8px",borderRadius:"8px"}}>›</button>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:"2px",marginBottom:"12px"}}>
              {weekDays.map((wd,i)=><div key={i} style={{textAlign:"center",fontSize:"10px",fontWeight:700,color:th.textMuted,padding:"4px 0"}}>{wd}</div>)}
              {cells.map((c,i)=><div key={i} style={{textAlign:"center",padding:"6px 0",fontSize:"12px",fontWeight:isToday(c.d)&&c.cur?800:500,color:!c.cur?th.textMuted:isToday(c.d)?'#fff':th.text,background:isToday(c.d)&&c.cur?th.accentGrad:'transparent',borderRadius:"8px",cursor:c.cur?"pointer":"default"}} onClick={()=>{if(c.cur){const d=new Date(calYear,calMonth,c.d);const loc2=navigator.language||'en-US';const stampText=d.toLocaleDateString(loc2,{weekday:'short',year:'numeric',month:'short',day:'numeric'})+' '+new Date().toLocaleTimeString(loc2,{hour:'2-digit',minute:'2-digit'});navigator.clipboard?.writeText(stampText);}}}>{c.d}</div>)}
            </div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingTop:"10px",borderTop:`1px solid ${th.border}`}}>
              <div style={{fontSize:"10px",color:th.textMuted}}>{tzName}<br/>{tzStr}</div>
              <button onClick={()=>{setTool(T.STAMP);setCalendarOpen(false);}} style={{padding:"8px 16px",borderRadius:"10px",border:"none",background:th.accentGrad,color:"#fff",fontSize:"11px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",boxShadow:th.glow}}>Stamp on Canvas</button>
            </div>
          </>;})()}
      </div>}
      ...
      <div style={{position:"absolute",bottom:compact?"10px":"16px",left:"50%",transform:"translateX(-50%)",zIndex:100,display:"flex",gap:compact?"6px":"10px",alignItems:"center",flexWrap:"wrap",justifyContent:"center",maxWidth:"calc(100vw - 32px)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"2px",background:th.toolbar,backdropFilter:blur,borderRadius:"16px",padding:"4px",border:`1px solid ${th.toolbarBorder}`,boxShadow:th.shadow}}>{[{fn:()=>{setShowGrid(!showGrid);setShowRuled(false);},icon:I.grid,active:showGrid,c:th.text},{fn:()=>{setShowRuled(!showRuled);setShowGrid(false);},icon:I.ruled,active:showRuled,c:th.text},"sep",{fn:manualSave,icon:I.save,c:th.success},{fn:()=>setShowUploadModal(true),icon:I.upload,c:th.accent},{fn:exportPng,icon:I.image,c:th.text},{fn:openEmailModal,icon:I.mail,c:th.accent},{fn:()=>setShowFeedback(true),icon:I.feedback,c:th.textSecondary},{fn:clearCanvas,icon:I.trash,c:th.danger}].map((a,i)=>a==="sep"?<div key={i} style={{width:"1px",height:"18px",background:th.border,margin:"0 2px"}}/>:(<button key={i} onClick={a.fn} style={{width:"30px",height:"30px",borderRadius:"9px",border:"none",background:a.active?th.accentGrad:"transparent",color:a.active?"#fff":a.c||th.textSecondary,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:a.active?th.glow:"none"}}>{a.icon}</button>))}</div>
      </div>
        ...
        {sidebarOpen&&<div style={{width:compact?"200px":"240px",background:th.surface,borderRight:`1px solid ${th.border}`,padding:`${compact?52:56}px 14px 14px`,overflowY:"auto",zIndex:90}}>
          ...
          {folders.map(fl=>{const folderPages=pages.map((pg,i)=>({...pg,_idx:i})).filter(pg=>pg.folderId===fl.id);return(
            <div key={fl.id} style={{marginBottom:"6px"}}>
              {/* Folder header */}
              <div style={{display:"flex",alignItems:"center",borderRadius:"8px",padding:"2px 0",cursor:"pointer",border:dragOverFolder===fl.id?`2px dashed ${th.accent}`:'2px dashed transparent',background:dragOverFolder===fl.id?th.accentSoft:'transparent',transition:"border .15s, background .15s"}} onDragOver={e=>{e.preventDefault();e.dataTransfer.dropEffect='move';setDragOverFolder(fl.id);}} onDragLeave={()=>setDragOverFolder(null)} onDrop={e=>{e.preventDefault();const pid=Number(e.dataTransfer.getData('text/plain'));if(pid){setPages(p=>p.map(pg=>pg.id===pid?{...pg,folderId:fl.id}:pg));setFolders(f=>f.map(f2=>f2.id===fl.id?{...f2,expanded:true}:f2));}setDragOverFolder(null);}} onMouseEnter={e=>{e.currentTarget.querySelectorAll('.fld-del,.fld-edit').forEach(b=>b.style.opacity='0.6');}} onMouseLeave={e=>{e.currentTarget.querySelectorAll('.fld-del,.fld-edit').forEach(b=>b.style.opacity='0');}}>
              ...
              {/* Pages in folder */}
              {fl.expanded&&<div style={{paddingLeft:"12px"}} onDragOver={e=>{e.preventDefault();e.dataTransfer.dropEffect='move';setDragOverFolder(fl.id);}} onDragLeave={()=>setDragOverFolder(null)} onDrop={e=>{e.preventDefault();const pid=Number(e.dataTransfer.getData('text/plain'));if(pid){setPages(p=>p.map(pg=>pg.id===pid?{...pg,folderId:fl.id}:pg));setFolders(f=>f.map(f2=>f2.id===fl.id?{...f2,expanded:true}:f2));}setDragOverFolder(null);}}>
                {folderPages.map(pg=>{const i=pg._idx;return(
                  <div key={pg.id} draggable onDragStart={e=>{e.dataTransfer.setData('text/plain',String(pg.id));e.dataTransfer.effectAllowed='move';}} onDragEnd={()=>setDragOverFolder(null)} style={{display:"flex",alignItems:"center",marginBottom:"1px",borderRadius:"8px",background:currentPage===i?th.surfaceHover:"transparent",cursor:"grab"}} onMouseEnter={e=>e.currentTarget.querySelector('.pg-del')&&(e.currentTarget.querySelector('.pg-del').style.opacity='0.6')} onMouseLeave={e=>e.currentTarget.querySelector('.pg-del')&&(e.currentTarget.querySelector('.pg-del').style.opacity='0')}>
          ...
          <div style={{marginTop:"24px",paddingTop:"16px",borderTop:`1px solid ${th.border}`,textAlign:"center"}}><div style={{fontSize:"13px",fontWeight:700,color:th.text,fontFamily:'"Literata",Georgia,serif',marginBottom:"8px"}}>Ink Notes</div><button onClick={()=>setShowFeedback(true)} style={{display:"inline-flex",alignItems:"center",gap:"5px",padding:"6px 14px",borderRadius:"10px",border:`1.5px solid ${th.border}`,background:"transparent",cursor:"pointer",fontSize:"11px",fontWeight:600,color:th.textMuted,fontFamily:"inherit"}}>{I.feedback} Feedback</button></div>
        </div>}
