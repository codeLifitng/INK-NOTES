...
export default function NoteApp({onHome}) {
  ...
  const hitTestShape=(pos)=>{for(let i=shapeInputs.length-1;i>=0;i--){const s=shapeInputs[i];const mnX=Math.min(s.x1,s.x2)-8,mnY=Math.min(s.y1,s.y2)-8,mxX=Math.max(s.x1,s.x2)+8,mxY=Math.max(s.y1,s.y2)+8;if(s.type===T.LINE||s.type===T.ARROW){const dx=s.x2-s.x1,dy=s.y2-s.y1,len=Math.sqrt(dx*dx+dy*dy);if(len<1)continue;const t=Math.max(0,Math.min(1,((pos.x-s.x1)*dx+(pos.y-s.y1)*dy)/(len*len)));const px=s.x1+t*dx,py=s.y1+t*dy,dist=Math.sqrt((pos.x-px)**2+(pos.y-py)**2);if(dist<12)return s;}else{if(pos.x>=mnX&&pos.x<=mxX&&pos.y>=mnY&&pos.y<=mxY)return s;}}return null;};
  ...
  const handleUp=(e)=>{
    ...
    if(dragShapeStart.current){const ds=dragShapeStart.current;dragShapeStart.current=null;const sh=siRef.current.find(s=>s.id===ds.id);if(sh)broadcast({t:'shp_upd',id:sh.id,obj:sh});return;}
