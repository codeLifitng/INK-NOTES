...
export default function NoteApp({onHome}) {
  ...
  const handleRemoteData=useCallback((msg)=>{
    ...
    else if(msg.t==='shape'){ctx.save();const ss={x:msg.x1,y:msg.y1},pos={x:msg.x2,y:msg.y2};if(msg.tool===T.LINE){ctx.strokeStyle=msg.c;ctx.lineWidth=msg.s;ctx.lineCap="round";ctx.beginPath();ctx.moveTo(ss.x,ss.y);ctx.lineTo(pos.x,pos.y);ctx.stroke();}else if(msg.tool===T.ARROW)drawArrow(ctx,ss.x,ss.y,pos.x,pos.y,msg.c,msg.s);else if(msg.tool===T.RECT){ctx.lineWidth=msg.s;ctx.lineCap="round";if(msg.f&&msg.f!=='transparent'){ctx.fillStyle=msg.f;ctx.fillRect(ss.x,ss.y,pos.x-ss.x,pos.y-ss.y);}ctx.strokeStyle=msg.c;ctx.strokeRect(ss.x,ss.y,pos.x-ss.x,pos.y-ss.y);}else if(msg.tool===T.DIAMOND)drawDiamond(ctx,ss.x,ss.y,pos.x-ss.x,pos.y-ss.y,msg.c,msg.s,msg.f);else if(msg.tool===T.CIRCLE){const rx=Math.abs(pos.x-ss.x)/2,ry=Math.abs(pos.y-ss.y)/2;ctx.lineWidth=msg.s;ctx.lineCap="round";ctx.beginPath();ctx.ellipse(ss.x+(pos.x-ss.x)/2,ss.y+(pos.y-ss.y)/2,rx,ry,0,0,Math.PI*2);if(msg.f&&msg.f!=='transparent'){ctx.fillStyle=msg.f;ctx.fill();}ctx.strokeStyle=msg.c;ctx.stroke();}ctx.restore();}
    else if(msg.t==='sync'){const img=new Image();img.onload=()=>{ctx.clearRect(0,0,PW,PH);ctx.drawImage(img,0,0,PW,PH);};img.src=msg.image;if(msg.texts)setTextInputs(msg.texts);}
    ...
    else if(msg.t==='clear'){ctx.fillStyle="#ffffff";ctx.fillRect(0,0,PW,PH);setTextInputs([]);}
  },[]);
  const createRoom=()=>{const code=Math.random().toString(36).substring(2,8).toUpperCase();setRoomCode(code);setPeerStatus('hosting');const p=new Peer('inknotes-'+code,{config:ICE});peerRef.current=p;p.on('connection',(conn)=>{connRef.current=conn;conn.on('open',()=>{setPeerStatus('connected');const cv=canvasRef.current;if(cv)conn.send({t:'sync',image:cv.toDataURL(),texts:tiRef.current});});conn.on('data',handleRemoteData);conn.on('close',()=>{connRef.current=null;setPeerStatus('hosting');});});p.on('error',(e)=>console.error('Peer:',e));};
  ...
  const handleUp=(e)=>{
    ...
    if([T.LINE,T.ARROW,T.RECT,T.DIAMOND,T.CIRCLE].includes(tool)&&shapeStart.current){const pos=getPos(e),ctx=canvasRef.current.getContext("2d");overlayRef.current?.getContext("2d").clearRect(0,0,PW,PH);ctx.save();const ss=shapeStart.current;if(tool===T.LINE){ctx.strokeStyle=color;ctx.lineWidth=brushSize;ctx.lineCap="round";ctx.beginPath();ctx.moveTo(ss.x,ss.y);ctx.lineTo(pos.x,pos.y);ctx.stroke();}else if(tool===T.ARROW)drawArrow(ctx,ss.x,ss.y,pos.x,pos.y,color,brushSize);else if(tool===T.RECT){ctx.lineWidth=brushSize;ctx.lineCap="round";if(fillColor!=='transparent'){ctx.fillStyle=fillColor;ctx.fillRect(ss.x,ss.y,pos.x-ss.x,pos.y-ss.y);}ctx.strokeStyle=color;ctx.strokeRect(ss.x,ss.y,pos.x-ss.x,pos.y-ss.y);}else if(tool===T.DIAMOND)drawDiamond(ctx,ss.x,ss.y,pos.x-ss.x,pos.y-ss.y,color,brushSize,fillColor);else if(tool===T.CIRCLE){const rx=Math.abs(pos.x-ss.x)/2,ry=Math.abs(pos.y-ss.y)/2;ctx.lineWidth=brushSize;ctx.lineCap="round";ctx.beginPath();ctx.ellipse(ss.x+(pos.x-ss.x)/2,ss.y+(pos.y-ss.y)/2,rx,ry,0,0,Math.PI*2);if(fillColor!=='transparent'){ctx.fillStyle=fillColor;ctx.fill();}ctx.strokeStyle=color;ctx.stroke();}ctx.restore();broadcast({t:'shape',tool:toolRef.current,x1:ss.x,y1:ss.y,x2:pos.x,y2:pos.y,c:colorRef.current,s:brushRef.current,f:fillRef.current});shapeStart.current=null;}
  ...
  useEffect(()=>{if(tool!==T.SELECT&&selection)commitSel();if(tool!==T.TEXT&&tool!==T.SELECT){commitEditing();setSelectedText(null);}},[tool]);
  ...
  const collectState=useCallback(()=>{
    ...
    pdRef.current=pd;
    return{version:8,folders:fldRef.current,pages:pgRef.current,currentPage:cpRef.current,pageData:pd,settings:{showGrid:sgRef.current,showRuled:srRef.current,dark:darkRef.current}};
  ...
  const clearCanvas=()=>{const ctx=canvasRef.current?.getContext("2d");if(!ctx)return;ctx.fillStyle="#ffffff";ctx.fillRect(0,0,PW,PH);broadcast({t:'clear'});setTextInputs([]);setEditingText(null);setSelectedText(null);setSelection(null);selSnap.current=null;saveHist();};
